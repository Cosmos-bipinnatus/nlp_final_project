...
class RouterDecision(BaseModel):
    ...
    """
    chosen_route: Literal["local", "arxiv"] = Field(
        ..., 
        description="決定將此提問路由分發至何處。若問及本地上傳文獻、已上傳特定論文的段落細節或具體數據，選 'local'；若問及外部前沿技術、搜尋最新論文、未上傳之廣泛研究主題，選 'arxiv'。"
    )
...
class AcademicRouterAgent:
    ...
    def __init__(self, model_name: str = "gemini-2.5-flash", persist_directory: str = "vectorstore"):
        """
    ...
    def make_decision(self, query: str, chat_history: List[dict] = None) -> RouterDecision:
        ...
        system_prompt = (
            ...
            "你的唯一任務是分析學生提出的問題，並決定該問題應該路由分發至「本地文獻庫 (local)」還是「外接 ArXiv 學術庫 (arxiv)」。\n\n"
            "【路由分發判定準則】\n"
            "1. 本地文獻庫 (local)：\n"
